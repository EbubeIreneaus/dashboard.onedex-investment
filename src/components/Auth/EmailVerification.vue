...
async function VerifyOTP(otp){
  ...
  try {
   ...
   return notify('error', 'OTP verification failed', res.code)
    ...
    <q-card class="full-width bg-accent text-white" style="max-width: 500px">
      <q-card-section>
        ...
        <div class="text-caption text-center q-pt-md">
          please enter the six(6) digit code sent to {{ EncodeEmail(props.email) }}
        </div>
      </q-card-section>
      ...
      <q-card-section>
        <div class="text-caption text-center">
          ...
          <q-btn flat unelevated dense :disabled="timer > 0" @click="requestOTP">{{
